...
import { Copy, FileText, Pencil, Printer, ThumbsUp, Trash2, ChevronUp, ChevronDown } from 'lucide-react';

export default function BlogPage({ blogs, onCreate, onEdit, onDelete }) {
  ...
  return (
    <div className="flex-1 overflow-y-auto bg-gray-100 p-4 sm:p-6">
      <div className="mb-4 flex items-center justify-between gap-3">
        <h1 className="text-lg font-bold text-gray-800">Blog Manage</h1>
        <button
          type="button"
          onClick={onCreate}
          ...
          <table className="w-full min-w-[720px] text-sm">
            <thead>
              <tr className="border-b border-gray-200 bg-slate-100">
                ...
                <SortableHeader label="Image" sortKey="imageColor" sort={sort} onSort={toggleSort} className="w-40" />
                <SortableHeader label="Status" sortKey="status" sort={sort} onSort={toggleSort} className="w-44" />
            ...
            <tbody>
              {filtered.length === 0 ? (
                ...
                filtered.map((blog, index) => (
                  <tr key={blog.id} className="border-b border-gray-100 transition hover:bg-gray-50">
                    <td className="px-4 py-4 text-gray-600">{index + 1}</td>
                    <td className="px-4 py-4 text-gray-700">{blog.title}</td>
                    <td className="px-4 py-4">
                      <div
                        className="flex h-12 w-12 items-center justify-center overflow-hidden rounded-full border border-gray-200 bg-cover bg-center text-center text-[8px] font-bold leading-tight text-white"
                        style={{ background: blog.imageColor }}
                      >
                        {blog.imageText}
                      </div>
                    </td>
                    <td className="px-4 py-4">
                      <span className={`rounded px-2 py-0.5 text-[11px] font-semibold ${
                        blog.status ? 'bg-emerald-100 text-emerald-600' : 'bg-rose-100 text-rose-500'
                      }`}
                      >
