...
import { Copy, FileText, Pencil, PlayCircle, Printer, Trash2, ChevronUp, ChevronDown } from 'lucide-react';
import { tagManagerService } from '../../services/marketingService';

export default function TagManagerPage({ onCreate, onEdit }) {
  ...
  const [sort, setSort]       = useState({ key: 'tagManagerId', dir: 'asc' });
  const limit = 20;

  const fetchItems = useCallback(async () => {
    setLoading(true); setError('');
    try {
      const res = await tagManagerService.getAll({ searchTerm: search || undefined, page, limit });
      setItems(res.data || []);
      setMeta(res.meta || {});
  ...
  async function handleDelete(item) {
    if (!window.confirm('Delete this tag manager?')) return;
    try { await tagManagerService.delete(item.Id); fetchItems(); }
  ...
  return (
    <div className="flex-1 overflow-y-auto bg-gray-100 p-4 sm:p-6">
      <div className="mb-4 flex items-center justify-between gap-3">
        <h1 className="text-lg font-bold text-gray-800">Tag Manager Manage</h1>
      ...
      <div className="rounded bg-white p-6 shadow-sm">
          ...
          <table className="w-full min-w-[720px] text-sm">
            <thead>
              <tr className="border-b border-gray-200 bg-slate-100">
                ...
                <SortableHeader label="Tag Manager ID" sortKey="tagManagerId" sort={sort} onSort={toggleSort} />
            ...
            <tbody>
              ...
              {!loading && sorted.map((item, i) => (
                <tr key={item.Id} className="border-b border-gray-100 transition hover:bg-gray-50">
                  <td className="px-4 py-3 text-gray-600">{(page - 1) * limit + i + 1}</td>
                  <td className="px-4 py-3 text-gray-700">{item.tagManagerId}</td>
                  <td className="px-4 py-3">
                    <span className={`rounded px-2.5 py-0.5 text-xs font-semibold ${item.status === 'Active' ? 'bg-green-100 text-green-700' : 'bg-gray-100 text-gray-600'}`}>
                      {item.status}
                    </span>
                  </td>
                  <td className="px-4 py-3">
                    <div className="flex items-center gap-1.5">
                      <button type="button" title="Edit" onClick={() => onEdit && onEdit(item)}
                        className="flex h-7 w-7 items-center justify-center rounded bg-blue-500 text-white transition hover:bg-blue-600"><Pencil size={13} /></button>
                      <button type="button" title="Delete" onClick={() => handleDelete(item)}
                        className="flex h-7 w-7 items-center justify-center rounded bg-red-500 text-white transition hover:bg-red-600"><Trash2 size={13} /></button>
                    </div>
                  </td>
                </tr>
              ))}
        ...
        <div className="flex items-center justify-between py-4 text-sm font-semibold text-gray-500">
          ...
          <div className="flex items-center gap-2 text-slate-400">
            <button type="button" onClick={() => setPage((p) => Math.max(1, p - 1))} disabled={page === 1} className="disabled:opacity-40 transition hover:text-indigo-600">‹</button>
            <button type="button" className="flex h-9 w-9 items-center justify-center rounded-full bg-indigo-600 text-sm font-semibold text-white">{page}</button>
            <button type="button" onClick={() => setPage((p) => Math.min(totalPages, p + 1))} disabled={page >= totalPages} className="disabled:opacity-40 transition hover:text-indigo-600">›</button>
          </div>
        </div>
      </div>
    </div>
