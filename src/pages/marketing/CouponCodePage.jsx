...
import { Copy, FileText, Pencil, PlayCircle, Printer, ThumbsDown, Trash2, ChevronUp, ChevronDown } from 'lucide-react';
import { couponCodeService } from '../../services/marketingService';

export default function CouponCodePage({ onCreate, onEdit }) {
  ...
  const fetchItems = useCallback(async () => {
    ...
    try {
      const res = await couponCodeService.getAll({ searchTerm: search || undefined, page, limit });
  ...
  async function handleDelete(item) {
    ...
    try { await couponCodeService.delete(item.Id); fetchItems(); }
    catch (err) { alert(err.message); }
  }

  async function handleToggleStatus(item) {
    try {
      await couponCodeService.update(item.Id, { status: item.status === 'Active' ? 'Inactive' : 'Active' });
      fetchItems();
    } catch (err) { alert(err.message); }
  }
  ...
  return (
    <div className="flex-1 overflow-y-auto bg-gray-100 p-4 sm:p-6">
      <div className="mb-4 flex items-center justify-between gap-3">
        <h1 className="text-lg font-bold text-gray-800">Coupon Code Manage</h1>
          ...
          <table className="w-full min-w-[720px] text-sm">
            <thead>
              <tr className="border-b border-gray-200 bg-slate-100">
                ...
                {[['code','Code'],['date','Date'],['type','Type'],['amount','Amount'],['buyAmount','Buy Amount'],['status','Status']].map(([key, label]) => (
                  <SortableHeader key={key} label={label} sortKey={key} sort={sort} onSort={toggleSort} />
                ))}
            ...
            <tbody>
              ...
              {!loading && error && <tr><td colSpan={8} className="px-4 py-8 text-center text-red-400">{error}</td></tr>}
              {!loading && !error && sorted.length === 0 && <tr><td colSpan={8} className="px-4 py-8 text-center text-gray-400 bg-gray-50">No data available</td></tr>}
              {!loading && sorted.map((item, i) => (
                <tr key={item.Id} className="border-b border-gray-100 transition hover:bg-gray-50">
                  <td className="px-4 py-4 text-center text-gray-600">{(page - 1) * limit + i + 1}</td>
                  <td className="px-4 py-4 text-gray-700 font-mono">{item.code}</td>
                  ...
                  <td className="px-4 py-4 text-gray-600">{item.buyAmount}</td>
                  <td className="px-4 py-4">
                    <span className={`rounded px-2 py-0.5 text-[11px] font-semibold ${item.status === 'Active' ? 'bg-emerald-100 text-emerald-600' : 'bg-rose-100 text-rose-500'}`}>{item.status}</span>
                  </td>
                  <td className="px-4 py-4">
                    <div className="flex items-center gap-3">
                      <button type="button" title="Toggle Status" onClick={() => handleToggleStatus(item)}
                        className="flex h-7 w-8 items-center justify-center rounded bg-gray-600 text-white transition hover:bg-gray-700"><ThumbsDown size={13} /></button>
                      <button type="button" title="Edit" onClick={() => onEdit && onEdit(item)}
                        className="flex h-7 w-8 items-center justify-center rounded bg-indigo-600 text-white transition hover:bg-indigo-700"><Pencil size={13} /></button>
                      <button type="button" title="Delete" onClick={() => handleDelete(item)}
                        className="flex h-7 w-8 items-center justify-center rounded bg-rose-500 text-white transition hover:bg-rose-600"><Trash2 size={13} /></button>
                    </div>
                  </td>
                </tr>
              ))}
