...
import { Copy, FileText, Pencil, Printer, Trash2, ChevronUp, ChevronDown } from 'lucide-react';

export default function ExpensePage({ expenses, onCreate, onEdit, onDelete }) {
  ...
  const filtered = useMemo(() => {
    return expenses
      .filter((expense) => `${expense.title} ${expense.category}`.toLowerCase().includes(search.toLowerCase()))
      .sort((a, b) => {
  ...
  return (
    <div className="flex-1 overflow-y-auto bg-gray-100 p-4 sm:p-6">
      <div className="mb-4 flex items-center justify-between gap-3">
        <h1 className="text-lg font-bold text-gray-800">Expense Manage</h1>
          ...
          <table className="w-full min-w-[720px] text-sm">
            <thead>
              <tr className="border-b border-gray-200 bg-slate-100">
                ...
                <SortableHeader label="Name" sortKey="title" sort={sort} onSort={toggleSort} />
            ...
            <tbody>
              {filtered.map((expense, index) => (
                <tr key={expense.id} className={`${index % 2 === 0 ? 'bg-gray-50' : 'bg-white'} border-b border-gray-100 transition hover:bg-gray-100/70`}>
                  <td className="px-4 py-4 text-gray-600">{index + 1}</td>
                  <td className="px-4 py-4 text-gray-600">{expense.date}</td>
                  <td className="px-4 py-4 text-gray-700">{expense.title}</td>
                  <td className="px-4 py-4 text-gray-600">{expense.category}</td>
                  <td className="px-4 py-4 text-gray-600">{expense.amount}</td>
                  <td className="px-4 py-4">
                    <div className="flex items-center gap-3">
                      <button type="button" title="Edit" onClick={() => onEdit(expense)} className="flex h-7 w-8 items-center justify-center rounded bg-indigo-600 text-white transition hover:bg-indigo-700">
                        <Pencil size={13} />
                      </button>
                      <button type="button" title="Delete" onClick={() => onDelete(expense.id)} className="flex h-7 w-8 items-center justify-center rounded bg-rose-500 text-white transition hover:bg-rose-600">
                        <Trash2 size={13} />
                      </button>
                    </div>
                  </td>
